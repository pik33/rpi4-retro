...
uses unix,baseunix, sysutils,classes,unit6502,
     retro, retromouse, retrokeyboard, mp3;
...
// system variables
    ...
    dupa:array[0..1920*1200] of byte;
...
mousetype:=0;
repeat
  repeat m:=getmousereport; sleep(2); until m[0]<>$7FFFFF;

  if mousedebugwindow<>nil then
    begin
    for i:=0 to 3 do mousedebugwindow.print(inttohex(m[i],8)+' ');
  ...
  if m[0]<>2 then begin offsetx:=0; offsety:=0; wheel:=0; end;
  x:=mousex+offsetx;
  if x<0 then x:=0;
...
//background.canvas:=pointer(mainscreen) ;//@dupa;
