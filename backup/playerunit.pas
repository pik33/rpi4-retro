...
uses
  Classes, SysUtils, retromalina, mwindows, blitter, {threads, simpleaudio,} retro, icons, retrokeyboard, unit6502,xmp,mp3,sdl;
...
else if key=ord('p') then begin pause1a:=not pause1a; if pause1a then sdl_pauseaudio(1) else sdl_pauseaudio(0); end
...
if (pl.mx>124) and (pl.mx<170) and (pl.my>176) and (pl.my<212) and (mousek=1) and (clickcount>60) and not pause_down and (pl.selected) then
  ...
  pause_down:=true;
  pause1a:=not pause1a; if pause1a then SDL_pauseaudio(1) else SDL_pauseaudio(0);
  if pause1a then
    begin
    blit8(integer(playpaus),66,0,integer(pl.canvas),48,56,6,18,84,550);      // clear transport status
