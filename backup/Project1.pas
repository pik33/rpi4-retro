...
//  blitter,
  retro,
  playerunit,
  mwindows,icons, sysinfo, calculatorunit, mandelbrot;
  //simpleaudio, {scripttest,} xmp, mwindows, calculatorunit, icons, sysinfo,
...
label p999;
const ver='Colors v. 0.31 --- 2019.07.04';
...
// background.cls(144);
//goto p999;
workdir:='/home/pi/colors/';
drive:='/home/pi/';
s:=drive+'colors/wallpapers/rpi-logo.rbm';
...
for i:=0 to 299 do
  for j:=0 to 234 do
    if (peek(scr+j+i*235)>15) or (peek(scr+j+i*235)<5) then poke (integer(@dupa){mainscreen}+xres*(i+(yres div 2)-150)+j+(xres div 2) - 117,peek(scr+j+i*235));
//for c:='C' to 'F' do drivetable[c]:=directoryexists(c+':\');
//TODO: assign drive letters to directories in Linux
songtime:=0;
siddelay:=20000;
startreportbuffer;
...
s:='/home/pi/colors/icons/Calculator-01.png';
calculator.LoadICONFromFile(s);    // added pjde
...
repeat
// background.box(0,0,1000,1000,40);

  background.icons.checkall;
  if lpeek(base+$60000)=120 then begin  box(0,0,1000,1000,40);   outtextxyz(100,100,inttostr(lpeek(base+$60000)),120,3,3); end;
