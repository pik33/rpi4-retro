...
uses
  Classes, SysUtils, retro, icons, retromalina;
...
type TIcon=class(TObject)
  ...
  //   procedure LoadICONFromStream (fn : string);   // added pjde
...
//    wt1:=gettime;
    ...
    repeat rectangle:=rectangle^.next until (rectangle^.handle=pointer(self.handle)) or (rectangle^.next=nil);
//    wt1:=gettime-wt1;
//    retromalina.box(0,0,100,32,0);retromalina.outtextxy(0,0,inttostr(wt1),15);
    if rectangle^.handle=pointer(self.handle) then
      begin
...
begin
 // s.Seek(0,soFromBeginning);
  im:=TFPMemoryImage.Create(48,48);
  try
   im.LoadFromfile(fn);
//   im.LoadFromStream(s);
   sz:=im.Width;
...
while window<>nil do
  ...
  while rect<>nil do begin
    if (rect^.x1>=x1) and (rect^.y1>=y1)and (rect^.x2<x2) and (rect^.y2<y2) and (rect^.handle=nil) then rect^.handle:=pointer(window);
    rect:=rect^.next;
    end;
