...
//------------------------------------------------------------------------------
//
// This is a version for RPi4 using sdl to do its job :(
//
//------------------------------------------------------------------------------
...
uses  Classes, SysUtils, Platform, HeapManager, Threads, GlobalConst, math, sdl;
...
procedure antialias(bufaddr,len:integer);
...
threadsleep(1);
repeat
  repeat threadsleep(1) until (dma_cs and 2) <>0 ;         // change to sdl_buffer_empty
  ...
  if pauseA>0 then  // clean the buffers
    begin
    if nc=nocache+ctrl1_adr then for i:=0 to 16383 do dmabuf1_ptr[i]:=CurrentAudioSpec.range div 2; //if emtybuffer=0  fill it with silence
    if nc=nocache+ctrl2_adr then for i:=0 to 16383 do dmabuf2_ptr[i]:=CurrentAudioSpec.range div 2; //              1
    ...
    if nc=nocache+ctrl1_adr then noiseshaper8a (samplebuffer_32_adr,dmabuf1_adr,CurrentAudioSpec.oversample,CurrentAudioSpec.samples)      // now these are intermediate buffers
    ...
    // we need to filter the buffer and make undersampling
    end;
